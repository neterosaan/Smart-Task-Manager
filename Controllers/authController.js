...
const createAndStoreRefreshToken = async (user) => {
  ...
  user.refreshTokenExpires = new Date(Date.now() + expiresInDays * 24 * 60 * 60 * 1000);
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) {
    return next(new AppError('the user belongin to this token does no longer exist.', 401));
  }
